        ...
        <div className="relative pb-12 md:pb-20">
          ...
          <div className="text-center">
              ...
              <div className="relative flex flex-col" ref={testimonials}>
                ...
                {items.map((item, index) => (
                  <Transition
                    ...
                    appear={true}
                  >
                    <div className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-zinc-200/60 via-zinc-200 to-zinc-200/60">{item.quote}</div>
                  </Transition>
                ))}
            ...
            <div className="flex flex-wrap justify-center -m-1.5">
              ...
              {items.map((item, index) => (
                <button className={`btn-sm m-1.5 text-xs py-1.5 text-zinc-300 transition duration-150 ease-in-out [background:linear-gradient(theme(colors.zinc.900),_theme(colors.zinc.900))_padding-box,_conic-gradient(theme(colors.zinc.400),_theme(colors.zinc.700)_25%,_theme(colors.zinc.700)_75%,_theme(colors.zinc.400)_100%)_border-box] relative before:absolute before:inset-0 before:bg-zinc-800/30 before:rounded-full before:pointer-events-none ${active === index ? 'opacity-100' : 'opacity-30 hover:opacity-60'}`} key={index} onClick={() => { setActive(index); setAutorotate(false); }}>
                  <span className="relative">
                    <span className="text-zinc-50">{item.name}</span> <span className="text-zinc-600">-</span> <span>{item.role}</span>
                  </span>
                </button>
              ))}
